...
import { fetchCarRental, fetchLocations, createLocationAsync, updateLocationAsync, } from "../redux/apiCall";
import { useNavigate } from "react-router-dom";


function EditeForm({ form, onChange, }) {
  ...
  useEffect(() => {
    if (form.immatricule) {
      console.log("Form immatricule:", form.immatricule);
      const car = cars.find((car) => car.immatricule === form.immatricule);
      console.log("Found car:", car);
      setSelectedCar(car || null);
    }
  }, [form.immatricule, cars]);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(createLocationAsync(data))
      ...
      dispatch(updateLocationAsync(data))
      .unwrap()
      .then(() => {
        ...
        <div className="d-flex justify-content-between align-items-center">
          <div className="me-3 flex-grow-1 col-3">
            ...
            <input
              ...
              name="startDate"
              defaultValue={formatDate(form?.startDate)}
              onChange={onChange}
          ...
          <div className="me-3 flex-grow-1 col-3">
            ...
            <input
              ...
              className="form-control"
              name="endDate"
              defaultValue={formatDate(form?.endDate)}
              onChange={onChange}
              placeholder="Demain"
            />
          </div>
          ...
          <div className="flex-grow-1 col-3">
            ...
            >
              ...
              {cars.length > 0 ? (
                cars.map((car) => (
                  <option key={car._id} value={car.immatricule}>
                    {car.immatricule || "Unknown"}
                  </option>
                ))
              ) : (
      ...
      <div className="row">
        ...
        <aside className="col-md-4 bg-light p-3">
          ...
          <div className="alert mt-3 text-center" style={{ fontSize: "50px" }}>
           <span style={{ fontSize: "25px" }}>{selectedCar?.price || 0} MAD</span>
          </div>
        </aside>
      </div>
    </div>
